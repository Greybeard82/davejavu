  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <p className="text-xs text-mid-gray mb-6">{tier.size} · Personal use · <strong className="text-orange">€{tier.price}</strong></p>
        ...
        <p className="text-[10px] text-mid-gray text-center mt-4">
          Personal use only.{' '}
          <Link href={`/${locale}/license`} className="underline hover:text-orange transition-colors">License terms</Link>
        </p>
      </div>
    </div>
  );
  ...
  if (status === 'success') {
    return (
      <div className="p-5 border border-[#d1d1d1] bg-[#f4f3ef] rounded-sm text-center">
        ...
        <p className="text-sm font-600 text-charcoal mb-1">Payment confirmed</p>
        <p className="text-xs text-mid-gray">Check your inbox — your download link will arrive within a minute.</p>
      </div>
    );
  }
    ...
    <PayPalScriptProvider options={PAYPAL_INIT}>
      <div className="space-y-3">
        {TIERS.map((tier) => (
          <button
            key={tier.key}
            onClick={() => { setStatus('idle'); setActiveTier(tier); }}
          ...
          >
            <div className="text-left">
              <p className="text-sm font-600 text-charcoal group-hover:text-orange transition-colors">{tier.label}</p>
              <p className="text-xs text-mid-gray mt-0.5">{tier.size} · Personal use</p>
            </div>
